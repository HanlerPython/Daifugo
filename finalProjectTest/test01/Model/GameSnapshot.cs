...
namespace test01.Model
{
    public class GameSnapshot
    {
        ...
        public int[] OpponentHandCounts { get; } // 其他玩家剩餘手牌張數

        // 新增：公開建構子，讓你可以在測試區 Mock，也讓隊友未來可以生成快照
        public GameSnapshot(Hands handsType, int weight, int count, bool isReversed, int[] oppCounts = null)
        {
            CurrentHandsType = handsType;
            CurrentWeight = weight;
            CurrentCardCount = count;
            IsReversed = isReversed;
            OpponentHandCounts = oppCounts ?? new int[3]; // 預設 3 個對手
        }
